...
**jupyter-mcp tools are the default** for live editing, executing, and managing cells in notebooks.
Use `nbdev` MCP tools when they are more efficient than jupyter-mcp:

- **`nb_search`** for searching across notebooks (jupyter-mcp has no search equivalent)
- **`nb_read`** for quickly reading a notebook or specific cells (faster than `use_notebook` + `read_notebook`)
- **`nb_run`** for batch execution (one call instead of many `execute_cell` calls)
- **`nb_create`** for scaffolding new notebooks
- **`nb_validate`** before commits

When jupyter-mcp is unavailable, first use `nb_mcp_url` to check the Jupyter server URL and token.
If there is no server or a persistent issue, use the `nbdev` MCP tools for all operations — but only
...
|---|---|
| `Grep`/`grep` a `.ipynb` file | `nb_search` MCP tool (preferred — jupyter-mcp has no search) |
| `Read`/`cat` a `.ipynb` file | `nb_read` MCP tool (preferred) or jupyter-mcp `read_notebook` |
| `Edit` a `.ipynb` file's JSON | jupyter-mcp `overwrite_cell_source` (preferred for live sessions) or `nb_edit` MCP tool |
| `Write` a new `.ipynb` from scratch | `nb_create` MCP tool |
| Write Python to parse/edit `.ipynb` | Use `nb_read`, `nb_edit`, `nb_cells`, or `nb_search` MCP tools — they already do it well! |
| Loop over `execute_cell` calls | `nb_run` MCP tool with upto or range params (one call, one kernel) |
...
Ensure the `jupyter-mcp` server is configured in `.mcp.json`.
...
2. **Check server is running**: `nb_mcp_url` MCP tool (preferred) or if necessary `uv run jupyter server list`
...
|---------|-------------|-----|
| MCP server not listed in `/mcp` | `$JUPYTER_TOKEN` not set when `claude` started | Use `nb_mcp_url` MCP tool to get the token |
| MCP connected but `use_notebook` fails | Jupyter server not running or wrong port | Use `nb_mcp_url` MCP tool to check the URL & port or `uv run jupyter server list` to confirm; restart if needed |
...
**Fall back to nbdev MCP tools** when ALL of these are true:
- The jupyter-mcp server or Jupyter itself won't start, or Y.js sync is persistently broken
...
3. Use `nb_read` MCP tool to verify the notebook state matches what you
   last saw via jupyter-mcp
4. Continue with nbdev MCP tools (see [Offline Mode](#offline-mode-nbdev-mcp-tools) below)
5. Remind the user to run `uv run nbdev_prepare` before committing, since you
   can no longer execute cells to verify

---

## Offline Mode — nbdev MCP Tools

> **Reminder:** Only use these tools to edit a notebook when jupyter-mcp is unavailable and the user has approved offline mode.
> All tools below are available via the `nbdev` MCP server.

### Reading Notebooks

| Situation | Tool | Why |
|-----------|------|-----|
| Understand notebook structure | `nb_read nb.ipynb --outline` | See all cells at a glance |
| Read full notebook content | `nb_read nb.ipynb` | Human-readable, shows cell types and directives |
| Read specific cells | `nb_read nb.ipynb --cell 5` | Focus on one area |
| See what's exported | `nb_read nb.ipynb --exports` | Quick view of the module's public API |
| See test coverage | `nb_read nb.ipynb --tests` | What's being tested |
...
| Situation | Tool | Why |
|-----------|------|-----|
| Find a class/function across notebooks | `nb_search nbs/ --pattern "class Foo"` | One line per matching cell with path and index |
| Case-insensitive search | `nb_search nbs/ --pattern "error" -i` | Flexible text matching |
| Find all exported cells | `nb_search nbs/ --directive export` | See every export across the project |
| Search only code cells | `nb_search nbs/ --pattern "import" --type code` | Filter out prose matches |
| See context around matches | `nb_search nbs/ --pattern "class Foo" -C 3` | Like `grep -C` with cell structure |
| Which notebooks mention X? | `nb_search nbs/ --pattern "MyClass" --files-only` | Like `grep -l` for notebooks |
...
| Situation | Tool | Why |
|-----------|------|-----|
| Run all cells in a notebook | `nb_run nb.ipynb` | Runs everything in one call |
| Run up to a specific cell | `nb_run nb.ipynb --upto 5` | Verify state builds correctly through cell 5 |
| Run a range of cells | `nb_run nb.ipynb --range 2-5` | Execute cells 2 through 5 in one call |
| Run a single cell | `nb_run nb.ipynb --cell 5` | Quick check of one cell |
| Run and save outputs | `nb_run nb.ipynb --upto 5 --save` | Persist outputs in the notebook file |
| Debug failing cells | `nb_run nb.ipynb --allow-errors` | Continue past errors to see all failures |
...
| Situation | Tool | Why |
|-----------|------|-----|
| New module notebook | `nb_create --module name --out path` | Correct metadata, structure, cell IDs |
| Complex notebook from spec | `nb_create --from spec.txt --out path` | Write content in plain text, get valid JSON |
...
| Situation | Tool | Why |
|-----------|------|-----|
| Small change within a cell | `nb_edit nb.ipynb --cell N --old "..." --new "..."` | Find-and-replace scoped to one cell, no need to rewrite the whole cell |
| Small change (find cell by content) | `nb_edit nb.ipynb --match "class Foo" --old "..." --new "..."` | When you know the content but not the index |
...
| Add multiple related cells | `nb_cells nb.ipynb insert --at N --from spec.txt` | One operation for function + prose + test |
| Append cells to end | `nb_cells nb.ipynb append --from spec.txt` | Simpler than calculating position |
| Move a cell | `nb_cells nb.ipynb move --from-pos N --to-pos M` | Reorder narrative flow |
| Remove cells | `nb_cells nb.ipynb remove --cells 3,5,7` | Batch removal |
| Find a cell to edit | `nb_cells nb.ipynb find --pattern "class Foo"` | Locate by content |
...
| Situation | Tool | Why |
|-----------|------|-----|
| Before committing | `nb_validate nbs/` | Catch structural errors |
| After raw JSON edits | `nb_validate notebook.ipynb` | Verify valid structure |
| Full project check | `nb_validate nbs/ --strict` | No warnings either |
