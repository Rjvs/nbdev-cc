...
This is an nbdev project. Notebooks in `{{NBS_PATH}}/` are the source of truth.
...
uv run nbdev_test --path {{NBS_PATH}}/00_core.ipynb   # Single notebook
uv run nbdev_test --flags slow                        # Include slow tests
```

| Path | Contents |
|------|----------|
| `{{NBS_PATH}}/` | Source notebooks (the source of truth) |
| `{{NBS_PATH}}/index.ipynb` | Becomes README.md and docs homepage |
